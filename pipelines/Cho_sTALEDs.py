...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', filename='logging_Cho_G1397_sTALEDs_with_Bystanders.log')
...
def process_mtDNA(mtDNA_seq, pos):
    ...
    if pos in T_positions:
            ...
            for window_size in range(14, 19): #for window sizes of 14-18bp long --> MAJOR ASSUMPTION!!
                if window_source == "sTALED with AD on the left_TALE":
                    ...
                    for num, window in enumerate(sTALED_left_T_windows, start=5):
                        ...
                        all_windows.append((PIPELINE ,window_source, pos, ref, mut, ws, final_window, window_desc, off_target_sites,combined_set if combined_set else {0}, TALES, FLAG))
        ...
        logger.warning("Base at position %d is not in a editable context and cannot be edited by this pipeline.", pos)
        print(f"Position {pos} is not in a editable context and cannot be edited by this pipeline. Please choose a valid position.")
...
def append_to_excel(all_windows, additional_file, output_file):
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl', mode='a' if os.path.exists(output_file) else 'w') as writer:
        ...
        all_windows_df = pd.DataFrame(all_windows, columns=['Pipeline', 'sTALED Type' ,'Position', 'Reference_Base', 'Mutant_Base', 'Window Size', 'Window Sequence', 'Description of window', 'Number of Bystanders', 'Position of Bystanders', 'Matching TALEs', 'Flag_CheckBystanderEffect'])
