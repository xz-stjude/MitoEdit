...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', filename='logging_Mok2020_G1333_with_Bystanders.log')
...
def generate_TC_windows(mtDNA_seq, pos, window_size):
    ...
    for i in range(4, 12): 
        if i != 11: 
            start_index = max(0, pos - i)  # starting index of window, adjusted to shift & added a 1 to make it a 1-indexed numbering
...
def generate_GA_windows(mtDNA_seq, pos, window_size):
    ...
    for i in range(4, 12):  # loop over position 4-10 from the left
        if i != 11: #excluding the 11th pos! 
            start_index = max(0, pos - window_size + i -1) #start index of window, adjusted to shift left --> do i need a -1 ? 
...
def find_TC_positions(window, start_position):
    ...
    while True:
        ...
        if index_tc == -1:
            break
        tc_positions.append(start_position + index_tc + 1+1)  # +1 for 1-indexed
...
def process_mtDNA(mtDNA_seq, pos):
    ...
    if pos in consecutive_TC_positions:
        ...
        #logger.info("The 60 adjacent bases to my target base are: %s", adjacent_bases) 
        ...
        for window_size in range(14, 19):
            ...
            for num, window in enumerate(TC_windows, start=4):
                ...
                if marked_window[int_pos + 1] == 'C': #TO DEAL WITH TCC context
...
def append_to_excel(all_windows, additional_file, output_file):
    ...
    new_data = filtered_df[['mtDNA_pos','Ref. Allele', 'Mutant Allele','Location', 'Predicted Impact', 'Syn vs NonSyn', 'AA Variant', 'Func. Impact', 'MutationAssessor Score']]
    new_data.columns = ['Bystander Position','Reference Base','Mutant Base','Location On Genome', 'Predicted Mutation Impact!', 'SNV_Type', 'AA_Variant', 'Functional Impact', 'MutationAssessor Score']
    with pd.ExcelWriter(output_file, engine='openpyxl', mode='a' if os.path.exists(output_file) else 'w') as writer:
        ...
        all_windows_df = pd.DataFrame(all_windows, columns=['Pipeline', 'Position', 'Reference_Base', 'Mutant_Base', 'Window Size', 'Window Sequence', 'Description of window', 'Number of Bystanders', 'Position of Bystanders', 'Matching TALEs', 'Flag_CheckBystanderEffect'])
